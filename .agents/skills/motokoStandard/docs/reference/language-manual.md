...
Actors and actor class declaration may specify a migration expression, using an optional, leading `<parenthetical>` expression with a required field named `migrationStandard`.
...
* Its `migrationStandard` field must be present and have a non-shared function type whose domain and codomain are both record types.
