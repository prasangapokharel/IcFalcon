...
Each migration's `migrationStandard` function declares which fields it reads (input) and which fields it produces (output). The relationship between input and output fields determines what happens to the state:
...
The corresponding actor declaration should no longer include `emailStandard`.
