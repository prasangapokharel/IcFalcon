...
**IcFalcon project workflow** (one-shot wire, `migrations/` modules, APPLIED): load [`../../migration/SKILL.md`](../../migration/SKILL.md).

**For multi-migration with a `migrations/` directory**, load `migratingMotokoEnhanced` instead.
...
- Load `writingMotoko` for general Motoko language reference
- Load `migratingMotokoEnhanced` for multi-migration with `--enhanced-migration`
