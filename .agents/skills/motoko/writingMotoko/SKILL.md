...
**For actor upgrades/migrations:** load `migratingMotoko` for inline migration or `migratingMotokoEnhanced` for multi-migration with `--enhanced-migration`. Under `--enhanced-migration`, actor fields **cannot** have initializers — declare them as `var x : T;` and set initial values in the migration that introduces them. The actor examples in this skill use initializers and would need adjustment for enhanced-migration projects.
...
| `Compatibility error [M0170]`                          | Load `migratingMotokoEnhanced`            |
